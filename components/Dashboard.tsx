...
export default function Dashboard({onLogout,username}:{onLogout:()=>void;username:string}){
 ...
 const header = district || division ? "DISTRICT" : state ? "DIVISION" : region ? "STATE" : "COUNTRY";
 const headerValue = district || division || state || region || "India";
 ...
 return <div className="app-bg">
  ...
  <main className="shell content">
   ...
   {tab==="table"&&<section className="card table-card"><div className="section-head"><div><div className="section-title">Detailed Report</div><div className="legend">{header}: <b>{headerValue}</b> • Source rows: {total.toLocaleString("en-IN")}</div></div><div className="legend"><Clock size={13} style={{verticalAlign:"middle"}}/> {date.toLocaleDateString("en-GB",{day:"2-digit",month:"2-digit",year:"numeric"})} • {date.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false})}</div></div>
    <div className="table-wrap"><table className="table"><thead><tr><th>{header}</th><th>Deeni Activities</th><th>Fields</th><th>Report Value</th><th>Target {target}%</th><th>Achievement</th><th>Region</th><th>State</th><th>Division</th><th>District</th></tr></thead><tbody>{rows.length?rows.map((r,i)=><tr key={i}><td><b>{r.name}</b></td><td>{r.deeniActivities||"-"}</td><td>{r.fields||"-"}{r.multipleFieldName&&<div style={{fontSize:9,color:"#64748b"}}>{r.multipleFieldName}: {r.multipleFieldValue||"-"}</div>}</td><td><b>{num(r.report)}</b></td><td>{num(r[currentTarget])}</td><td className={(r[currentAch]||0)>=100?"good":"bad"}>{pct(r[currentAch])}</td><td>{r.region||"-"}</td><td>{r.state||"-"}</td><td>{r.division||"-"}</td><td>{r.district||"-"}</td></tr>):<tr><td colSpan={10} className="empty">{loading?"Loading report...":"No data found. Select a Report Month & Year."}</td></tr>}</tbody></table></div>
   </section>}
